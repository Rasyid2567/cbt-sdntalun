...
if (isset($_GET['action']) && in_array($_GET['action'], ['export_pdf', 'export_doc', 'export_dokumen', 'export_jawaban'], true)) {
    ...
    if ($totalSoal <= 7) {
        ...
        $p1 = (int)ceil($totalSoal / 2);
        $p2 = $totalSoal - $p1;
        $pageDefinitions[] = [
            "type" => "first",
            "soal" => array_slice($soalList, 0, $p1),
            "has_header" => true,
            "has_signature" => false,
        ];
        $pageDefinitions[] = [
            "type" => "last",
            "soal" => array_slice($soalList, $p1),
            "has_header" => false,
            "has_signature" => true,
        ];
    } elseif ($totalSoal <= $p1Cap + $pLastCap) {
        // <= 25 butir soal: 2 halaman (Halaman 1: 12 soal, Halaman 2: sisa soal + tanda tangan)
        $p1 = $p1Cap;
        $p2 = $totalSoal - $p1;
        $pageDefinitions[] = [
            "type" => "first",
        ...
        // > 25 butir soal:
        // Halaman 1 memuat Kop, Identitas Siswa, Ringkasan Nilai, dan 12 butir soal pertama
        $pageDefinitions[] = [
            "type" => "first",
            "soal" => array_slice($soalList, 0, $p1Cap),
            "has_header" => true,
            "has_signature" => false,
        ];
        ...
        while ($rem > 0) {
            if ($rem <= $pLastCap) {
                ...
                $offset += $rem;
                $rem = 0;
            } elseif ($rem <= $pLastCap + $pmCap) {
                // Sisa butuh 2 halaman (1 halaman tengah + 1 halaman penutup tanda tangan)
                if ($rem > $pmCap) {
                    $mCount = $pmCap;
                    $lCount = $rem - $mCount;
                } else {
                    $lCount = min($pLastCap, max(3, $rem - 14));
                    $mCount = $rem - $lCount;
                }
                $pageDefinitions[] = [
                    "type" => "middle",
                ...
                // Masih banyak soal tersisa, isi halaman tengah penuh 18 soal
                $pageDefinitions[] = [
                    "type" => "middle",
                    "soal" => array_slice($soalList, $offset, $pmCap),
                    "has_header" => false,
                    "has_signature" => false,
                ];
                $offset += $pmCap;
